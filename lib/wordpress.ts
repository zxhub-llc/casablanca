...
/**
 * Fetches up to 100 recent posts. For fetching ALL posts (e.g., sitemap),
 * use getAllPostsForSitemap() instead.
 */
...
/**
 * Fetches ALL posts with proper pagination for sitemap generation.
 * Iterates through all pages to ensure complete coverage.
 */
export async function getAllPostsForSitemap(): Promise<Post[]> {
  ...
  try {
    ...
    console.warn("WordPress unavailable for sitemap generation");
    return [];
  }
}
